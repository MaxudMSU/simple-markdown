...
NumList::NumList(const NewString& str, wchar_t* marker, int nesting){
  nesting_ = nesting;
  //копируем числовой маркер
  marker_ = new wchar_t[100];
  int i = 0;
  while (marker[i] != L'\0'){
    *(marker_ + i) = marker[i];
    i++;
...
int NumList::print([[maybe_unused]] const DisplayParameters& display) const{
  ...
  if (display.in_file)
    ...
    wprintf(L"\033[96m%ls\033[0m", marker_);
  
  int shift = wcslen(marker_) + nesting_*2;
